<?php
      ...
      //drupal_add_js(base_path() . path_to_theme() . '/js/jquery.mobile-1.3.1.min.js', 'external');
        ...
        drupal_add_js('window.onload = function() { parent.iframeLoaded(); }', 'inline');
        //drupal_add_js('function sembunyi() { var element = document.getElementById("navigation"); element.parentNode.removeChild(element); }', 'inline');
        drupal_add_js('var $j = jQuery.noConflict(); function sembunyi() { $j("#navigation").remove(); $j("nav.cbp-spmenu").remove(); $j("div#footer").remove(); $j("div.divider.bawah").remove(); $j("#content").addClass("nomargin"); $j("div.region-content").css({ padding: "0" }); }', 'inline');
?>
